...
router.get("/", (_, res) => {
  const classes = db.prepare("SELECT * FROM group_classes").all();
...
router.post("/:id/book", authenticateToken, (req: AuthRequest, res) => {
  ...
  if ((cls.spots_left as number) <= 0) {
    res.status(400).json({ error: "Plus de places disponibles" });
    return;
  }

  const existing = db.prepare(
    "SELECT id FROM class_bookings WHERE user_id = ? AND class_id = ?"
  ).get(userId, id);

  if (existing) {
    res.status(400).json({ error: "Deja inscrit a ce cours" });
    return;
  }
...
router.delete("/:id/book", authenticateToken, (req: AuthRequest, res) => {
  ...
  if (!booking) {
    res.status(400).json({ error: "Pas inscrit a ce cours" });
    return;
  }
